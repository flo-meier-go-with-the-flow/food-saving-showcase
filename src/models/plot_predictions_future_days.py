...
if __name__ == '__main__':
    ...
    with open(os.path.join('predictions_future_days', 'x_data_' + label_0 + '.pkl'), 'rb') as file:
        x_data_11_30 = pickle.load(file)
    with open(os.path.join('predictions_future_days', 'y_data_' + label_0 + '.pkl'), 'rb') as file:
        y_data_11_30 = pickle.load(file)
    with open(os.path.join('predictions_future_days', 'y_predictions_' + label_0 + '.pkl'), 'rb') as file:
        y_predictions_11_30 = pickle.load(file)
    label_1 = 'label_num_people_12_33'
    with open(os.path.join('predictions_future_days', 'x_data_' + label_1 + '.pkl'), 'rb') as file:
        x_data_12_33 = pickle.load(file)
    with open(os.path.join('predictions_future_days', 'y_data_' + label_1 + '.pkl'), 'rb') as file:
        y_data_12_33 = pickle.load(file)
    with open(os.path.join('predictions_future_days', 'y_predictions_' + label_1 + '.pkl'), 'rb') as file:
    ...
    if plot_first_error_visualization:
        ...
        for i in range(0, 1):
            ...
            plt.plot([ 11.5, 12.33, 14],
                     [num_people_11_30_data, num_people_12_33_data, menus_sold_data], 'ro')

            plt.errorbar([11.5, 12.33, 14],
                         [num_people_11_30_prediction, num_people_12_33_prediction, menus_sold_prediction],
                         yerr=[21.7, 20.1, 21.9], capsize=5.0, fmt='bo')
            plt.legend(['data', 'predictions'])
            plt.ylim(0, 300)


            plt.text(13.1, 113, r'Verkaufte Menus', fontsize=9)
            plt.text(13.4, 180, r'Error 21.7', fontsize=9, color='green')
            plt.text(11.9, 199, r'Error 20.1', fontsize=9, color='green')
            plt.text(11.0, 212, r'Error 21.9', fontsize=9, color='green')
            plt.xlim(8, 18)
            plt.xlabel('time')
            ...
            plt.title(fig_title)
            fig_id = '0_Predictions_future_days_' + str(day)
            save_fig(fig_id)
            plt.clf()

    plot_errors_subplots = True
    if plot_errors_subplots:
        ...
        for i_num, j in enumerate([0, 1, 2, 3, 6, 7, 8, 9]):
            ...
            plt.title(str(day))
            plt.grid(True)
        fig_id = '0_Predictions_future_days_subplots_' + str(start_day)
        save_fig(fig_id)
