...
def objective(params):
    ...
    (rmse, mae, r2), *_ = train_model(params)
...
def train_model(params):
    ...
    rmse_mean, mae_mean, r2_mean = np.mean(errors, axis=0)
    print('params:')
    print(params)
    print(f'rmse : {rmse_mean}')  # , mae : {mae_mean} , r2 : {r2_mean}')
    return (rmse_mean, mae_mean, r2_mean), pipe_estimator, coefficients
...
if __name__ == '__main__':
    ...
    bin_features = []  #'month_quarter_0','month_quarter_1','month_quarter_2','month_quarter_3','Spring/Autumn','Summer','Winter', 'Rain_binary','Spring/Autumn','Summer','Winter','zurich_vacation', 'zuehlke_day']
    ...
    search_space_elastic_net = {
        'alpha': hp.loguniform('alpha', -3, 3),  # if I understood this correctly it returns a value beteen e^a and e^b
    ...
    mlflow.set_experiment('11_30_to_menus_sold_experimentation')  # 'elastic_net_experimentation','elastic_net_label_12_20','elastic_net_label_11_30','elastic_net_label_num_menu_sold'
    #    mlflow.set_experiment('random_forest_reg')
    # mlflow.set_experiment('SVR')

    with mlflow.start_run():
        (rmse, mae, r2), estimator, coefficients = train_model(best_params)
        ...
        with open(os.path.join('pickled_models', num_features[0] + '_to_' + label[0] + '.pkl'), 'wb') as file: #num_features[0] + '_to_' + label[0] + '
            pickle.dump(final_estimator,file)

        make_predictions=False
        if make_predictions:
            with open(os.path.join('predictions', 'x_data_' + label[0]+'.pkl'), 'wb') as file:
                pickle.dump(x_data, file)
            with open(os.path.join('predictions', 'y_data_' + label[0]+'.pkl'), 'wb') as file:
                pickle.dump(y_data, file)
            with open(os.path.join('predictions', 'y_predictions_' + label[0]+'.pkl'), 'wb') as file:
